...
public class SuggestionsHandlerHelper<T>
{
    ...
    private void configureListeners()
    {
        ...
        componentFieldListener = (observable, oldValue, newValue) -> {
            ...
            List<T> filteredSuggestions =
                suggestions.stream()
                    .filter(item
                            -> filterFunction.apply(item).toLowerCase().contains(
    ...
    {
        int    itemCount   = Math.min(suggestionListView.getItems().size(),
                                 Constants.SUGGESTIONS_MAX_ITEMS);
    ...
    {
        suggestionsPopup.show(componentField,
