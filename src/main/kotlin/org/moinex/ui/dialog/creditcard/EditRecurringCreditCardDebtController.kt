/*
 * Filename: EditRecurringCreditCardDebtController.kt
...
@Controller
class EditRecurringCreditCardDebtController(
    ...
    private var recurringDebt: RecurringCreditCardDebt? = null
    ...
    fun setRecurringDebt(recurring: RecurringCreditCardDebt) {
        recurringDebt = recurring
        fillFields(recurring)
    }

    @FXML
    private fun handleSave() {
        val existing = recurringDebt ?: return
        ...
        runCatching {
            ...
            val updated =
                RecurringCreditCardDebt(
                    ...
                    endDate = endDate,
                    nextInvoiceMonth = existing.nextInvoiceMonth,
                    status = status,
                )
    ...
    private fun fillFields(recurring: RecurringCreditCardDebt) {
        crcComboBox.value = creditCards.find { it.id == recurring.creditCard.id }
        categoryComboBox.value = categories.find { it.id == recurring.category.id }
        frequencyComboBox.value = recurring.frequency
        descriptionField.text = recurring.description ?: ""
        valueField.text = recurring.amount.toPlainString()
        dayOfMonthField.text = recurring.dayOfMonth.toString()
        activeCheckBox.isSelected = recurring.isActive()

        if (recurring.endDate != Constants.RECURRING_TRANSACTION_DEFAULT_END_DATE) {
            endDatePicker.value = recurring.endDate
        }
    }
